...
export async function downloadBlogPost(ctx: ResponseContext, postName: string): Promise<void> {
	...
	const zip = await zipAwsFilesWithPrefix(
		`cycle/blog/posts/${post.name}/`,
...
async function zipAwsFilesWithPrefix(
	prefix: string,
	fileZippedCallback: (fileNumber: number, totalFileCount: number) => void
) {
