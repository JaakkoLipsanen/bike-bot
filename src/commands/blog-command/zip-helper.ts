...
export async function askForBlogZip(ctx: ResponseContext) {
    ...
    return await ctx.askForMessage<{ postName: string, zipBuffer: Buffer }>("Upload a blog .zip file", {
        accept: async (msg, reject) => {
            ...
            const missingParts = parts.map((value, index) => value ? "" : ((index + 1) + "o" + parts!.length)).filter(x => x);
            return reject(`${missingParts.length > 1 ? "Parts " : "Part "} ${missingParts.join(", ")} missing!`);
        }	
    }, "edit");
}
